{
  ...
  // --- Update UI Content based on index ---
  function updateDisplay(index) {
    ...
    // Handle image and text swap with a simple fade transition
    if (abilityMainImg) {
  ...
  abilityNavItems.forEach((item, idx) => {
    item.addEventListener("click", () => {
      updateDisplay(idx);
      resetAutoPlay(); // Restart timer on manual click
    });
  });

  // --- Auto-play Logic (5s interval) ---
